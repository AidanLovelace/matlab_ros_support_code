...
classdef ObjectIdentifier
    ...
    methods
        ...
        function this = ObjectIdentifier(robotHandle, options)
            ...
            this.imageDetectors   = { ...
                                        parallel.pool.Constant(@() load("../../vision_tutorials/detectors/detector_gral_sim.mat").detector), ...
                                        parallel.pool.Constant(@() load("../../vision_tutorials/detectors/detector_can_sim.mat").detector), ...
                                        parallel.pool.Constant(@() load("../../vision_tutorials/detectors/detector_pouch_sim.mat").detector) ...
                                    };
        ...
        function [objects, ptCloud] = identifyObjectsHere(this, zoneInspect)
            ...
            numOfObjects = sum(numOfObjects);
            % Get transforms to points of interest
            W_T_R = this.robotHandle.getModelPose('robot');
            ...
            for i = 1:numOfObjects
                ...
                if this.options.ComparePicksWithGazebo
            ...
            % extraObjects = this.getZoneStaticObjects(zoneInspect);
        ...
        function [ptCloud_pic, nonPlane_pic, ptCloud_world] = getSeveralPointClouds(this, locations)
            ...
            % Gripper pose
            mat_R_T_G = this.robotHandle.currentPose();
            ...
            for iter = 1:length(locations)
                ...
                if iter < 5
                    displaceG = this.robotHandle.offsetGripper(mat_R_T_G, locations{iter}, 0.12);

                    % These motions rotate the gripper inwards at end of displacement
                    % to center object.
                elseif iter > 5
                    displaceGA = this.robotHandle.offsetGripper(mat_R_T_G, locations{iter}, 0.12, 1, 0.05);
                end
            ...
            this.robotHandle.moveToPose(cur_gripper_location);
        ...
        function [future, imgMsg, myImg] = getLabeledImageAsync(this)
            ...
            future(1:size(this.imageDetectors,2)) = parallel.FevalFuture;

            for i = 1:size(this.imageDetectors,2)
                future(i) = parfeval(@threadedYOLODetection, 5, myImg, this.imageDetectors{i});
            end
        end
        ...
        function [model, topCenterPosition, pickPose, newLabel] = fitPickPoseToObject(this, objectPtCloud, label, bbox)
            ...
            try
                ...
                if strcmp(label, "can") || strcmp(label, "bottle")
                    ...
                    pickPosition = [-model.Center(2), model.Center(1), objectPtCloud.ZLimits(1, 2)];
                    pickPose = transl(pickPosition(1), pickPosition(2), pickPosition(3)) * trotx(-pi);

                    logPrint(3, 'fitPickPose-'+string(label), 6, '', "Pick Position: [%.3f, %.3f, %.3f]", pickPosition(1), pickPosition(2), pickPosition(3));

                    % Store top center position for visualization/debugging

                    % Transform pick pose from world frame to robot base frame
                    W_T_R2 = ros2matlabPose(this.robotHandle.getBasePose(), 1, 0, this.options);

                    pickPose = W_T_R2 \ pickPose;

                    % Offset gripper position slightly above object
                    pickPose(3, 4) = pickPose(3, 4) + 0.13;
        ...
        function [modelName, modelPos, dist] = debugGetNearestModelFromGazebo(this, pos)
            ...
            if isempty(cachedModelPoses) || isempty(cachedModelPosesLastUpdated) || isempty(cachedModelNames) || (nowtime - cachedModelPosesLastUpdated > 10000)
                ...
                allModels = this.robotHandle.getModels();
                ...
                for i = 1:size(cachedModelNames, 1)
                    ...
                    model = this.robotHandle.getModelPose(cachedModelNames{i});
